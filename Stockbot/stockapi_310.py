...
class StockAPI:
    ...
    def plot(self, stock_number, day):
        ...
        _, axes = mpf.plot(df.iloc[21:], type='candle', style=s, figratio=(16,9), xrotation=0, addplot=ap, volume=True,returnfig=True)
    ...
    def realtime(self, stock_number):
        ...
        #         raise KeyError('Time is invalid.')
        if 0 <= now.weekday() <= 4 and ((now.hour == 8) or (now.hour == 7 and now.minute >= 50) or (now.hour == 9 and now.minute <= 10)):
            raise KeyError('Time is invalid.')
        # 開盤前會清除網頁
        stock_data = {
            "name": [],
    ...
    def calculate_william_value(self, stock_number):
        ...
        return round(float(william_value.iloc[0]), 2)
